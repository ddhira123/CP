...
pair<int, int> solve(int mask) {
  ...
  for (int nex : adj[mask]) {
    if (nex & 1) continue;
    pair<int, int> kanan = solve(nex);
    pair<int, int> kiri = solve(mask - nex);
    pair<int, int> val = combat(kiri, kanan);

    // printf("kiri %d %d\n", kiri.first, kanan.first);
    // printf("kanan %d %d\n", kanan.first, kanan.second);
    // printf("ret %d %d\n", ret.first, ret.second);
    // printf("val %d %d\n", val.first, val.second);

    if (ret.first != 0) ret = val;
    else if (val.first == 0) {
...
int main() {
  ...
  pair<int, int> ret = solve((1 << (1 << n)) - 1);
  // printf("ret %d %d\n", ret.first, ret.second);
